...
def dot_product(a,b):
    ...
    out = None
    ### YOUR CODE HERE
    out = np.dot(a,b) ### 'np.dot()'function is used to get the dot product between 1-D vectors
    pass
    ### END YOUR CODE
    return out
...
def complicated_matrix_function(M, a, b):
    ...
    out = None
    ### YOUR CODE HERE
    arr = np.matmul(a,b) ### Matmul (Matrix multiplication) is used to get the matrix product of two arrays
    arr_T = np.transpose(a) ### Using numpy transpose for "a"
    out = np.matmul ((arr) * M, arr_T)
...
def eigen_decomp(M):
    ...
    ### YOUR CODE HERE
    # pass
    w,v = np.linalg.eig(M) ###Getting the eigenvalues and eigenvectors by the 'np.linalg.eig()'
    ### END YOUR CODE
    return w, v
...
def get_eigen_values_and_vectors(M, k):
    ...
    positions = eigenvalues.argsort()[::-1] ###Sorting eigenvalues by "argsort" function
